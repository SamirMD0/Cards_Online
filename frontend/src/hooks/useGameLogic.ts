...
export function useGameLogic(roomId: string | undefined) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "socket-auth-failed",
      handleAuthFailed as EventListener
    );
 ...
 // --- EFFECT 2: Event Listeners (FIXED) ---
useEffect(() => {
  if (!roomId || !userId) return;

  // ✅ Create stable handler closures that read from refs
...
}, [roomId, userId, navigate]); // ✅ CRITICAL: Remove showNotification and requestHand

  // --- EFFECT 3: Timer Countdown ---
  // ✅ GOOD: Memoize dependencies, only re-create when turn actually starts
  useEffect(() => {
    if (!gameState?.gameStarted || !isMyTurn) return;
  ...
  }, [gameState?.gameStarted, isMyTurn]); // ✅ Only when turn ownership changes
